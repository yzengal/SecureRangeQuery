...
void GetInputQuery(const std::string& fileName, std::vector<Rectangle_t>& queries) {
    ...
    fin >> qn;
    if (fin.fail()) {
        std::cout << "Failed to parse the query number " << fileName << std::endl;
        abort();
    }

    queries.resize(qn);
    for (int i=0; i<qn; ++i) {
        fin >> queryType >> queries[i].x >> queries[i].y >> quries[i].dx >> queries[i].dy;
        if (fin.fail()) {
...
void GetInputQuery(const std::string& fileName, std::vector<Circle_t>& queries) {
    ...
    fin >> qn;
    if (fin.fail()) {
        printf("Failed to parse the query number %\n", fileName.c_str());
        abort();
    }

    queries.resize(qn);
    for (int i=0; i<qn; ++i) {
        fin >> queryType >> queries[i].x >> queries[i].y >> quries[i].rad;
        if (fin.fail()) {
            std::cout << "Failed to parse the %d-th query " << i+1 << std::endl;
            abort();
        }  
